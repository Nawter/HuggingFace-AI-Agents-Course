...
from smolagents import CodeAgent, WebSearchTool, tool, GoogleSearchTool
...
def save_screenshot(memory_step: ActionStep, agent: CodeAgent) -> None:
    ...
    if driver is not None:
        ...
        try:
            ...
            image.save(f"screenshot_step_{current_step}.png")  # <-- ADD THIS LINE
...
def search_item_ctrl_f(text: str, nth_result: int = 1) -> str:
    ...
    if not elements:
        return f"No matches found for '{text}' on this page."
    if nth_result > len(elements):
        return f"Only {len(elements)} matches found for '{text}', but match {nth_result} was requested."
...
def initialize_agent(model):
    ...
    return CodeAgent(
        tools=[GoogleSearchTool(provider="serpapi"), go_back, close_popups, search_item_ctrl_f],
        model=model,
...
helium_instructions = """
Use your google search tool when you want to get Google search results.
...
helium_instructions += """
Do NOT import Key from helium. To scroll to the top of a page, just use:
scroll_up(num_pixels=10000)
"""

helium_instructions += """
When calling final_answer(), always use triple quotes for multi-line strings:
